...
**Version**: 0.2
**Date**: 2026-04-20
**Status**: Revised post-T-01 to reflect real Claude Code skills registration
**Author**: architect-subagent (dispatched by TPM during iteration 001 bootstrap loop 0; revised by architect-subagent during T-01 hot revision)
**Source PRD**: `prd/spec-001.md` v0.3
**Constitution**: `.constitution.md` (ratified 2026-04-19)
**Prior version**: `.skspec/.erd/history/plan-001-v0.1-pre-skills-revision.md`
**Revision reason**: T-01 validation against upstream SHA `c118c1c30f961921e41891df3318a2ccd2ceea54` discovered Claude Code registers commands as **agent skills** under `.claude/skills/<dir>/SKILL.md` per the `agentskills.io` spec, not flat `.claude/commands/<name>.md`. Sections §1.3, §3, §4 (all subsections), §18 OQ-3, and §19 Wave 4/5 notes were rewritten accordingly. See `.skspec/.prd-pending/accepted-as-erd-revision-20260420T050821Z.md` for the accepted proposal and `docs/ERD-VALIDATION.md` for the evidence.
...
- `resolve_upstream_command(sk_name: str) -> Path` — maps `/sk-specify` back to the upstream `specify` template source (`speckit/templates/commands/specify.md`). Takes the hyphenated skill stem (`sk-specify`), strips the `sk-` prefix, and returns the upstream template path.
- `resolve_upstream_skill(sk_name: str) -> Path` — when the upstream integration has *already* rendered its version as a Claude skill (post `specify init --ai claude`), this helper resolves the rendered `.claude/skills/speckit-<x>/SKILL.md` path. Useful when Harness commands want to hand off to or cross-reference the inherited `/speckit-*` skill in-place.
- `rename_artifact(src: Path, iteration: str) -> Path` — maps `spec.md` → `spec-00N.md` and redirects `specs/<branch>/` → `.skspec/prd/`.
- `inherit_body(upstream_md: Path, harness_header: dict) -> str` — returns the full text of a Harness `SKILL.md` built from an upstream template or upstream-rendered skill. Contract:
  1. **Read** upstream source (either raw template under `templates/upstream/commands/<x>.md` or rendered skill at `.claude/skills/speckit-<x>/SKILL.md`).
  2. **Strip** the upstream frontmatter entirely. The upstream body is the only portion the Harness inherits.
  3. **Translate** path/command references in the body:
     - `/speckit-<x>` → `/sk-<x>` where the Harness is wrapping an inherited command
     - `templates/commands/<x>.md` references remain pointing at upstream; the Harness does not re-source its inherited body
     - `$ARGUMENTS` placeholders are preserved verbatim (they are honored by Claude Code — see §4.2)
  4. **Emit** new frontmatter per the `agentskills.io` skill spec (§4.2), using `harness_header` dict as the source of truth for `name`, `description`, `argument-hint`, and `metadata.source`. Inject Claude-specific flags (`user-invocable: true`, `disable-model-invocation: false`) to match the registrar's post-processing contract.
  5. **Concatenate** `---\n<new frontmatter>\n---\n\n<translated body>\n` and return the resulting string. Caller writes this to `.claude/skills/sk-<x>/SKILL.md`.

This is the **only** place upstream paths are referenced AND the **only** place frontmatter translation happens. When upstream renames a command, a key, or a spec field, we fix one file. This preserves the Constitution §VI "one-file concentration" principle even though the adapter's scope has grown from "path rewrite" to "path rewrite + frontmatter reformat" (see T-11 impact note in §19).
...
│       ├── sk-status/SKILL.md
│       ├── sk-log/SKILL.md
│       ├── sk-iteration/SKILL.md
│       ├── sk-help/SKILL.md
│       ├── sk-tasks-retry/SKILL.md
│       └── sk-agent/SKILL.md
│
└── tests/
    ├── unit/
...
**Design principle**: all new Python code lives in `sk_harness/`; all new templates in `templates/harness/`; all new command registrations in `.claude/skills/sk-*/SKILL.md` (one directory per skill, following the `agentskills.io` spec adopted by Claude Code — see §4). The upstream `speckit/` and `templates/upstream/` trees are read-only from the fork's perspective.

**Note on upstream-rendered skills**: when a user runs `specify init --ai claude` (upstream's own bootstrap), upstream also writes `.claude/skills/speckit-<x>/SKILL.md` files for its own 9 commands. Those are upstream-owned and must not be edited by sk; the Harness wrappers live at `.claude/skills/sk-<x>/SKILL.md` alongside them. Constitution §VI is preserved because the Harness additions are wholly new directories under `.claude/skills/`, with zero overlap with upstream's `speckit-*` entries.

---

## 4. Command Registration

### 4.1 Registration mechanism — Claude Code agent skills

Claude Code does NOT read flat files at `.claude/commands/<name>.md` for its primary (skills-mode) integration. It picks up **agent skills** from `.claude/skills/<dir-name>/SKILL.md`, following the [`agentskills.io`](https://agentskills.io/specification) specification that upstream `spec-kit` adopted.

Evidence (from the pinned upstream tree at SHA `c118c1c30f961921e41891df3318a2ccd2ceea54`):

```python
# speckit/specify_cli/integrations/claude/__init__.py, lines 41-54
class ClaudeIntegration(SkillsIntegration):
    key = "claude"
    config = {
        "name": "Claude Code",
        "folder": ".claude/",
        "commands_subdir": "skills",           # ← directory is "skills", not "commands"
        ...
    }
    registrar_config = {
        "dir": ".claude/skills",               # ← target root
        "format": "markdown",
        "args": "$ARGUMENTS",                  # ← $ARGUMENTS IS honored
        "extension": "/SKILL.md",              # ← each skill is a directory + SKILL.md
    }
```

```python
# speckit/specify_cli/integrations/base.py, SkillsIntegration.build_command_invocation
def build_command_invocation(self, command_name: str, args: str = "") -> str:
    """Skills use ``/speckit-<stem>`` (hyphenated directory name)."""
    stem = command_name
    if "." in stem:
        stem = stem.rsplit(".", 1)[-1]
    invocation = f"/speckit-{stem}"             # ← hyphen, no dot
```

Consequences for the Harness:

- **Layout**: each `/sk-<name>` command lives in its own directory: `.claude/skills/sk-<name>/SKILL.md`. There is no `.claude/commands/` subtree for sk.
- **Naming on disk**: the skill directory name uses hyphens (`sk-init`, `sk-tasks-retry`), produced by substituting dots → hyphens in the logical command name (mirroring upstream's `skill_name = f"speckit-{template_name.replace('.', '-')}"`).
- **User-facing invocation**: `/sk-init`, `/sk-plan`, `/sk-tasks-retry`. Claude Code surfaces the skill by its directory stem. There is no dot-delimited user surface.
- **Upstream parallel**: inherited commands that the user pulls in via upstream `specify init --ai claude` register as `/speckit-specify`, `/speckit-plan`, etc., under `.claude/skills/speckit-<name>/SKILL.md`. sk wrappers coexist as sibling directories, never overwriting upstream entries.

### 4.2 Frontmatter format

Each `.claude/skills/sk-<name>/SKILL.md` opens with YAML frontmatter conforming to the `agentskills.io` spec plus Claude-specific flags injected by `ClaudeIntegration.post_process_skill_content`:

```markdown
---
name: "sk-specify"
description: "Draft or amend the PRD for the current or next iteration"
compatibility: "Requires spec-kit project structure with .specify/ directory, plus .skspec/ for the sk Harness layer"
metadata:
  author: "sk-harness"
  source: "templates/harness/commands/sk-specify.md"
...
Allowed frontmatter keys (Harness conformance target, all derived from upstream behavior):

| Key | Purpose | Source in upstream |
|--|--|--|
| `name` (required) | skill directory stem, matches `<dir>` | `CommandRegistrar.build_skill_frontmatter` |
| `description` (required) | one-line summary shown in skill picker | same |
| `compatibility` | declarative prerequisite string | same |
| `metadata.author` | free-form attribution | same |
| `metadata.source` | path back to the template that rendered this skill | same |
| `argument-hint` | hint text displayed inline during invocation | `ClaudeIntegration.inject_argument_hint` (lines 57-103) |
| `user-invocable` | boolean; Claude-specific; must be `true` for user-invocable slash commands | `ClaudeIntegration.post_process_skill_content` |
| `disable-model-invocation` | boolean; Claude-specific; normally `false` for sk commands | same |
| `scripts` | optional; shell invocations passed to the skill runtime | upstream templates use this for setup scripts (e.g., `plan.md` frontmatter) |

**Explicitly NOT used**: `allowed-tools:`. It is not a recognized skill-frontmatter field in the `agentskills.io` spec Claude Code implements. The Harness does not include it in any `/sk-*` skill. (This resolves OQ-3 in the negative — see §18.)

**Explicitly preserved**: `$ARGUMENTS`. The `registrar_config.args` value (line 52 of `ClaudeIntegration`) is `"$ARGUMENTS"`, which confirms Claude Code's skill runtime substitutes user argv into the body verbatim. Harness command bodies use `"$ARGUMENTS"` the same way upstream does.

### 4.3 Command catalog & rename mapping (v1)

Per PRD §6, v1 ships 20 unique skill directories covering the functional set below (counts: 3 setup + 6 SDD + 5 execution + 3 observation + 2 iteration/help + 2 override; the SDD `sk-tasks` directory doubles as the Observation listing surface via internal mode dispatch, keeping the physical directory count at 20):

| Group | Logical command | On-disk skill dir | User-facing invocation |
|--|--|--|--|
| Setup | `sk.init` | `.claude/skills/sk-init/SKILL.md` | `/sk-init` |
| Setup | `sk.constitution` | `.claude/skills/sk-constitution/SKILL.md` | `/sk-constitution` |
| Setup | `sk.config` | `.claude/skills/sk-config/SKILL.md` | `/sk-config` |
| SDD | `sk.specify` | `.claude/skills/sk-specify/SKILL.md` | `/sk-specify` |
| SDD | `sk.clarify` | `.claude/skills/sk-clarify/SKILL.md` | `/sk-clarify` |
| SDD | `sk.plan` | `.claude/skills/sk-plan/SKILL.md` | `/sk-plan` |
| SDD | `sk.tasks` | `.claude/skills/sk-tasks/SKILL.md` | `/sk-tasks` |
| SDD | `sk.analyze` | `.claude/skills/sk-analyze/SKILL.md` | `/sk-analyze` |
| SDD | `sk.checklist` | `.claude/skills/sk-checklist/SKILL.md` | `/sk-checklist` |
| Execution | `sk.go` | `.claude/skills/sk-go/SKILL.md` | `/sk-go` |
| Execution | `sk.pause` | `.claude/skills/sk-pause/SKILL.md` | `/sk-pause` |
| Execution | `sk.resume` | `.claude/skills/sk-resume/SKILL.md` | `/sk-resume` |
| Execution | `sk.step` | `.claude/skills/sk-step/SKILL.md` | `/sk-step` |
| Execution | `sk.review` | `.claude/skills/sk-review/SKILL.md` | `/sk-review` |
| Observation | `sk.status` | `.claude/skills/sk-status/SKILL.md` | `/sk-status` |
| Observation | `sk.log` | `.claude/skills/sk-log/SKILL.md` | `/sk-log` |
| Iteration | `sk.iteration` | `.claude/skills/sk-iteration/SKILL.md` | `/sk-iteration` |
| Iteration | `sk.help` | `.claude/skills/sk-help/SKILL.md` | `/sk-help` |
| Override | `sk.tasks.retry` | `.claude/skills/sk-tasks-retry/SKILL.md` | `/sk-tasks-retry` |
| Override | `sk.agent` | `.claude/skills/sk-agent/SKILL.md` | `/sk-agent` |

Mode dispatch (`list|show|graph` for `/sk-tasks`; `list|complete|show` for `/sk-iteration`; `list|add|remove` for `/sk-agent`; `show|set|edit` for `/sk-config`) is parsed inside the body by the invoked CLI subcommand (§4.4), not by a separate skill directory per mode. `sk-tasks-retry` gets its own directory because the dotted logical name `sk.tasks.retry` maps to the hyphenated directory stem `sk-tasks-retry`, which is a distinct slash command from `/sk-tasks`.

**Dual naming rule** (important for implementers and for the knowledge base — see `.knowledge/decisions.md`):

- **User-facing** (slash invocations, help text, UI copy): hyphenated — `/sk-init`, `/sk-tasks-retry`, `/sk-agent`.
- **Internal** (task IDs, decision-log `actor` names, state-JSON keys, programmatic references in Python source and docs): dot-delimited — `sk.init`, `sk.tasks.retry`, `sk.agent`. This matches the PRD v0.3 vocabulary, keeps grep-style search consistent across the code base, and tracks how upstream refers to its own commands internally (`specify.plan`, `specify.tasks`, etc.) even though its user surface is `/speckit-plan`.

Engineer subagents MUST use `.replace('.', '-')` when converting an internal command name to a skill directory, and the opposite mapping (`.replace('-', '.')` with the `sk-` / `sk.` prefix swap) when logging a slash invocation into a decision-log entry under an internal `actor=`. The adapter module (§1.3) owns these conversions.

### 4.4 Inheritance flow — how `/sk-plan` delegates to upstream `/speckit-plan` logic

The adapter module (§1.3) lets each Harness skill that wraps inherited logic remain short (the frontmatter + 5–30 lines of Harness-specific preamble). The body is produced by `inherit_body()`, which translates both the path references and the frontmatter.

**Concrete before/after**:

*Upstream source* — `templates/upstream/commands/plan.md` (post-fork; mirror of `speckit/templates/commands/plan.md`):
...
<detailed planning-workflow body per upstream …>
```

*After upstream renders this via `ClaudeIntegration.setup`* — on a vanilla `specify init --ai claude` run, `.claude/skills/speckit-plan/SKILL.md` contains:
...
*Harness-authored wrapper* — what `sk_harness.speckit_adapter.inherit_body("plan", harness_header)` returns, and what T-17 (the "inherited SDD commands" task in §19) writes to `.claude/skills/sk-plan/SKILL.md`:

```markdown
---
name: "sk-plan"
description: "Run upstream planning discipline against the active iteration's ERD location and per-iteration naming"
compatibility: "Requires spec-kit project structure with .specify/ directory, plus .skspec/ for the sk Harness layer"
metadata:
  author: "sk-harness"
  source: "templates/harness/commands/sk-plan.md (wraps templates/upstream/commands/plan.md)"
argument-hint: "Optional guidance for the planning phase"
user-invocable: true
disable-model-invocation: false
---
...
Apply the upstream `/speckit-plan` planning discipline against the **current active iteration** (`.iterations.json.active`). The produced plan file is `.erd/plan-00N.md` rather than upstream's default `specs/<branch>/plan.md`; `sk_harness.speckit_adapter.rename_artifact` performs the path translation before the underlying body writes anything.
...
<detailed planning-workflow body inherited verbatim from upstream, with `/speckit-tasks` handoff references rewritten to `/sk-tasks` per the adapter's translation step 3>
```

Key properties of the inheritance flow:

1. **Zero logic duplication**: the upstream planning body appears once — copied by the adapter at generation time, not re-authored.
2. **Frontmatter is synthesized, not copied**: the adapter drops the upstream `handoffs:` and `scripts:` keys (which target upstream's bootstrap layout) and emits a clean Harness frontmatter per §4.2.
3. **Path and command references are translated**: any body text referencing `/speckit-<x>`, `specs/<branch>/`, or upstream scripts is rewritten to its Harness equivalent; this is the fragile surface that must be tested per T-11.
4. **`$ARGUMENTS` is passed through untouched** because Claude Code's skill runtime substitutes it for both upstream and Harness skills identically.
5. **Idempotence**: regenerating `.claude/skills/sk-plan/SKILL.md` from the same upstream source + same `harness_header` dict yields byte-identical output, which is test-verifiable via fixture golden comparison in T-11.

Inherited commands in v1 (handled by `inherit_body`): `sk-clarify`, `sk-plan`, `sk-tasks`, `sk-analyze`, `sk-checklist` (5 directly inherited from upstream; upstream's `implement` command is explicitly *not* wrapped — sk replaces it with `/sk-go`'s TPM loop). Fresh Harness commands (15) are authored under `templates/harness/commands/<name>.md` and rendered through the same adapter pipeline, just without an upstream-source input.

### 4.5 Command invocation contract

Every `/sk-*` skill body, after its preamble, invokes a Python subprocess:

```
python -m sk_harness.cli <command> [args...]
```

The CLI produces **structured output** (JSON over stdout, prefixed with `SK:`) that the skill body parses and surfaces to the user. This makes the Python layer testable independently of Claude Code, and is the integration boundary with §9's batch dispatcher.
...
**Status**: **Resolved 2026-04-20** per T-01 evidence. See `docs/ERD-VALIDATION.md` rows 11–13 and `.skspec/.prd-pending/accepted-as-erd-revision-20260420T050821Z.md`.

**Resolution**:
- Claude Code's skills-mode integration (which sk targets) does NOT honor `allowed-tools:` in skill frontmatter. That key belongs to older flat-command registrations (e.g., `.gemini/commands/*.md`), not to the `agentskills.io` spec Claude Code adopted. **Drop `allowed-tools` from every Harness skill body.** If a Harness command needs to gate tool access, it must do so via prompt-level guidance in the body text, not via frontmatter metadata.
- Claude Code's skill runtime DOES honor `$ARGUMENTS` substitution — confirmed by `ClaudeIntegration.registrar_config["args"] = "$ARGUMENTS"` (line 52 of `speckit/specify_cli/integrations/claude/__init__.py`). **Keep `$ARGUMENTS` in every Harness skill body that accepts user arguments.**
- Supported/required frontmatter keys are enumerated in §4.2.
- The prior v0.1 "Options A/B/C" framing no longer applies: the decision is imposed by Claude Code's own skill loader, not chosen by sk.
...
Wave 4: Subagent templates & adapter (3 tasks)
  T-11  sk_harness.speckit_adapter + sk_harness.templating
        [v0.2 NOTE] Per §1.3 and §4.4, this task's scope now includes
        frontmatter *reformat* for the inheritance flow, not just path rewrite.
        Complexity raised from S/M → M/L. The adapter must emit
        agentskills.io-compliant frontmatter (§4.2) and drop upstream
        `handoffs:` / `scripts:` keys. The canonical tasks-001.md sidecar
        for this task is being updated by the TPM to reflect the widened
        scope and golden-fixture tests.
...
Wave 5: Commands (7 tasks — one per command file group)
        [v0.2 NOTE] All Wave 5 tasks emit outputs at
        `.claude/skills/sk-<name>/SKILL.md` (directory + SKILL.md per the
        agentskills.io spec), not `.claude/commands/sk.<name>.md`. Skill
        directory stems use hyphens (`sk-init`, `sk-tasks-retry`). Internal
        task-ID, decision-log `actor=`, and state-JSON references still use
        the dotted form (`sk.init`). See §4.3 for the full rename table.
        The canonical tasks-001.md sidecars for T-14 through T-20 are being
        updated by the TPM separately to list the corrected output paths.
  T-14  .claude/skills/sk-init/SKILL.md + sk_harness.cli subcommand
  T-15  sk-constitution + sk-config
  T-16  sk-specify (incl. --amend, --accept, --reject, .prd-pending flow)
  T-17  sk-clarify + sk-plan + sk-tasks + sk-analyze + sk-checklist (inherited via §4.4)
